...
const typeColors: Record<string, string> = {
  ...
  invoice: 'bg-rose-500/20 text-rose-400',
          ...
          {notifications.map((n) => {
            ...
            return (
              <div
                ...
                className={`bg-slate-800/60 border rounded-xl p-4 flex items-start gap-3 transition-colors cursor-pointer ${
                  n.is_read ? 'border-slate-700/40 opacity-70' : 'border-slate-700/60 hover:bg-slate-700/40'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className={`text-sm font-medium ${n.is_read ? 'text-slate-400' : 'text-slate-100'}`}>{n.title}</p>
