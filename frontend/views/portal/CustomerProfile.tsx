...
const CustomerProfile: React.FC = () => {
  ...
  if (error) return <div className="p-6 max-w-3xl mx-auto"><div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-4 text-rose-300 text-sm">{error}</div></div>;
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {saveMsg && (
        <div className={`mb-5 p-3.5 border rounded-xl text-sm ${saveMsg.includes('successfully') ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300' : 'bg-rose-500/10 border-rose-500/20 text-rose-300'}`}>
          {saveMsg}
        </div>
      )}

      <form onSubmit={handleSave} className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              value={form.email || ''}
              readOnly
              className="w-full h-10 px-3 bg-slate-800/30 border border-slate-700/40 rounded-lg text-sm text-slate-500 cursor-not-allowed"
            />
          </div>
          <div>
      ...
      <form onSubmit={handlePasswordChange} className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-6">
        ...
        {passwordMsg && (
          <div className="mb-4 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-sm text-emerald-300">{passwordMsg}</div>
        )}
        {passwordError && (
          <div className="mb-4 p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg text-sm text-rose-300">{passwordError}</div>
        )}
