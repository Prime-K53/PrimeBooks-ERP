...
--    so downloads return the full record shape.
...
-- 4. Index for company-scoped sync queries (already created in the original
--    migration — kept here for fresh deployments that only run this file).
...
--    databases where this file is applied standalone, recreate them.
DO $$
BEGIN
    IF NOT EXISTS (
